...
//stack for pushing/popping transformation states
class mStack {
    ...
    public:
        ...
        mStack (int num = 32) {
            ...
            matrices = new Matrix4d[num];
        
        }
    
        ~mStack() { delete[] matrices; }
    
    bool push(Matrix4d mat) {
        if (index == size - 1) {
            return false;
        }
...
public:
    ...
    void drawTree(Matrix4d model_transform, Material trunk, Material flowerball) {
        for (int i = 0; i < 8; i++) {
            model_transform *= rotation(i * sin(graphicsState.animation_time) / 90, Vector3d(0, 0, 1));
            
            mstack.push(model_transform);
            
            model_transform *= scale(0.4, 1, 0.4);
            m_cube->draw(graphicsState, model_transform, trunk);
            
            model_transform = *mstack.pop();
            
            model_transform *= translation(0, 1.0f, 0);
        }
        
        model_transform *= scale(1.5, 1.5, 1.5);
        model_transform *= rotation(8 * sin(graphicsState.animation_time) / 90, Vector3d(0, 0, 1));
        m_sphere->draw(graphicsState, model_transform, flowerball, 3);
        
        model_transform = *mstack.pop();
    }
    
    void drawWings(int lr, Matrix4d model_transform, Material greyPlastic) {
        mstack.push(model_transform);
        model_transform *= translation(0, 0.5, lr * 0.5);
        model_transform *= rotation(lr * sin(2* graphicsState.animation_time), Vector3d(1, 0, 0));
        model_transform *= scale(1.5, 0.2, 3);
        model_transform *= translation(-1, 0.5, lr * 0.5);
        m_cube->draw(graphicsState, model_transform, greyPlastic);
        model_transform = *mstack.pop();
    }
    
    void drawLegs (int lr, Matrix4d model_transform, Material greyPlastic, Material purplePlastic)
    {
        // BEE LEGS
        mstack.push(model_transform);
        model_transform *= translation(0, -0.5, lr * 0.5);
        model_transform *= rotation(lr * cos(graphicsState.animation_time)/2, Vector3d(1, 0, 0));
        Matrix4d moreleg_transform = model_transform;
        model_transform *= scale(0.2, 0.7, 0.1);
        model_transform *= translation(-4, -0.5, lr * 0.5);
        m_cube->draw(graphicsState, model_transform, greyPlastic);
        
        //Matrix4d moreleg_transform = model_transform;
        moreleg_transform *= translation(-0.8, -0.7, lr * 0.05);
        moreleg_transform *= rotation(lr * cos(graphicsState.animation_time)/8, Vector3d(1,0,0));
        moreleg_transform *= scale(0.2, 0.7, 0.1);
        moreleg_transform *= translation(0, -0.5, 0);
        m_cube->draw(graphicsState, moreleg_transform, purplePlastic);
        
        
        model_transform *= translation(-3, 0, 0);
        m_cube->draw(graphicsState, model_transform, greyPlastic);
        moreleg_transform *= translation(-3,0,0);
        m_cube->draw(graphicsState, moreleg_transform, purplePlastic);
    ...
    void drawBee(Matrix4d model_transform, Material purplePlastic,
                 Material greyPlastic, Material beeyellow)
    {
        //BEE
        mstack.push(model_transform);
        model_transform *= rotation(graphicsState.animation_time/2, Vector3d(0, 1, 0));
        model_transform *= translation(cos(graphicsState.animation_time),
                                       8 + 2*sin(graphicsState.animation_time),10);
        
        // BEE HEAD
        mstack.push(model_transform);
        model_transform *= scale(0.5,0.5,0.5);
        m_sphere->draw(graphicsState, model_transform, purplePlastic, 3);
        model_transform = *mstack.pop();
        
        // BEE BODY
        //square part
        mstack.push(model_transform);
        model_transform *= translation(-1.5,0, 0);
        model_transform *= scale(2, 1, 1);
        m_cube->draw(graphicsState, model_transform, greyPlastic);
        model_transform = *mstack.pop();
        // elipse part
        mstack.push(model_transform);
        model_transform *= translation(-4, 0, 0);
        model_transform *= scale(2, 0.7, 0.7);
        m_sphere->draw(graphicsState, model_transform, beeyellow, 3);
        model_transform = *mstack.pop();
        
        
        // BEE WING
        
        drawWings(1, model_transform, greyPlastic);  //RIGHT
        drawWings(-1, model_transform, greyPlastic); //LEFT
        
        
        // BEE LEGS
        
        drawLegs(1, model_transform, greyPlastic, purplePlastic); //RIGHT
        drawLegs(-1, model_transform, greyPlastic, purplePlastic); //LEFT
	...
	{		
		...
		Material purplePlastic( Vector4f( .9f,.5f,.9f,1 ), .2f, .8f, .5f, 40, "" ), 	// Supply an empty string if you want no texture
			     greyPlastic( Vector4f( .5f,.5f,.5f,1 ), .5f, 1, 1, 30, "" ),
			     earth( Vector4f( .2f,.2f,.2f,1 ), .5f, 1, .5f, 40, "earth.tga" ),
			     stars( Vector4f( .2f,.2f,.2f,1 ), .5f, 1, 1, 40, "stars.tga" ),
                 ground( Vector4f( 0.0f,0.8f,0.0f,1 ), .5f, 1, 1, 40, ""),
                 trunk( Vector4f(0.5f, 0.2f, 0.165f, 1), .5f, 1, 1, 40, ""),
                 beeyellow(Vector4f(1.0f, 1.0f, 0, 1), .5f, 1, 1, 30, ""),
                 flowerball(Vector4f(1.0f, 0, 0, 1), .5f, 1, 1, 30, "");
		...
		**********************************/
        ...
        model_transform *= translation(0, -10, -40);
        model_transform *= scale(100, 1, 100);
        // GROUND
        m_cube->draw (graphicsState, model_transform, ground);
        model_transform = *mstack.pop();
        model_transform *= translation(0, -10, -40);
        mstack.push(model_transform);
        
        drawTree(model_transform, trunk, flowerball);

        drawBee(model_transform, purplePlastic, greyPlastic, beeyellow);
